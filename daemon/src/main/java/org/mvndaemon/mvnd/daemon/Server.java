...
public class Server implements AutoCloseable, Runnable {
  ...
  public Server() {
    ...
    SocketFamily socketFamily =
        Environment.MVND_SOCKET_FAMILY
            .asOptional()
    ...
    try {
      ...
      Arrays.stream(Environment.values())
          ...
          .forEach(
              envKey ->
                  envKey.asOptional().ifPresent(val -> opts.add(envKey.getProperty() + "=" + val)));
    ...
    try (DaemonConnection connection = new DaemonConnection(socket)) {
      ...
      if (message instanceof BuildRequest buildRequest) {
        ...
        if (Boolean.getBoolean("mvnd.dump.client.env")) {
          // Environment can contain passwords or tokens, so do not dump, unless specifically asked
          // for
  ...
  private void expirationCheck() {
    if (expirationLock.tryLock()) {
      ...
      LOGGER.warn(
          "Previous DaemonExpirationPeriodicCheck was still running when the next run was"
              + " scheduled.");
    }
  }
  ...
  private void onExpire(String reason, DaemonExpirationStatus status) {
    ...
    registry.storeStopEvent(
        new DaemonStopEvent(daemonId, System.currentTimeMillis(), status, reason));
  }
  ...
  private void handle(DaemonConnection connection, BuildRequest buildRequest) {
    updateState(Busy);
    final BlockingQueue<Message> sendQueue =
        new PriorityBlockingQueue<>(64, Message.getMessageComparator());
        ...
        Connection.setCurrent(
            new Connection() {
              ...
              public <T extends Message> T request(
                  Message request, Class<T> responseType, Predicate<T> matcher) {
                try {
                  synchronized (recvQueue) {
