...
/**
 * Set the system properties, library.mvndnative.path, library.mvndnative.name, appropriately so
 * that mvndnative can find *.dll, *.jnilib and *.so files, according to the current OS (win, linux,
 * mac).
 *
 * <p>The library files are automatically extracted from this project's package (JAR).
 *
 * <p>usage: call {@link #initialize()} before using mvndnative.
 */
public class MvndNativeLoader {
   ...
   *
   * @return True if mvndnative native library is successfully loaded; false otherwise.
   */
  ...
  private static boolean extractAndLoadLibraryFile(
    ...
    String extractedLibFileName =
        String.format("mvndnative-%s-%s-%s", getVersion(), uuid, libraryFileName);
    ...
    try {
      ...
      try (InputStream nativeIn =
          MvndNativeLoader.class.getResourceAsStream(nativeLibraryFilePath)) {
        try (InputStream extractedLibIn = new FileInputStream(extractedLibFile)) {
          String eq = contentsEquals(nativeIn, extractedLibIn);
          if (eq != null) {
            throw new RuntimeException(
                String.format(
                    "Failed to write a native library file at %s because %s",
                    extractedLibFile, eq));
          }
        }
      }
      ...
      if (loadNativeLibrary(extractedLibFile)) {
        nativeLibrarySourceUrl =
            MvndNativeLoader.class.getResource(nativeLibraryFilePath).toExternalForm();
        return true;
      }
    } catch (IOException e) {
  ...
  private static void loadMvndNativeLibrary() throws Exception {
    ...
    if (mvndnativeNativeLibraryPath != null) {
      String withOs =
          mvndnativeNativeLibraryPath + "/" + OSInfo.getNativeLibFolderPathForCurrentOS();
    ...
    mvndnativeNativeLibraryPath =
        String.format("/%s/%s", packagePath, OSInfo.getNativeLibFolderPathForCurrentOS());
    boolean hasNativeLib =
        hasResource(mvndnativeNativeLibraryPath + "/" + mvndnativeNativeLibraryName);
    ...
    if (hasNativeLib) {
      ...
      if (extractAndLoadLibraryFile(
          mvndnativeNativeLibraryPath, mvndnativeNativeLibraryName, tempFolder)) {
        loaded = true;
        return;
      } else {
        triedPaths.add(mvndnativeNativeLibraryPath);
  ...
  public static String getVersion() {
    ...
    URL versionFile =
        MvndNativeLoader.class.getResource(
            "/META-INF/maven/org.mvndaemon.mvnd/mvnd-native/pom.properties");
